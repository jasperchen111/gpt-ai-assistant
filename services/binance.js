...
const generateSignature = (queryString) => {
  return crypto
    .createHmac('sha256', config.BINANCE_SECRET_KEY || '')
    .update(queryString)
    .digest('hex');
};
...
export const getTicker = async (instId = 'BTC-USDT') => {
  ...
  return {
    ...
    data: [{
      ...
      open24h: response.data.openPrice,
...
export const getHistoryCandles = async (instId = 'BTC-USDT', bar = '1H', after = '', before = '', limit = 100) => {
  ...
  if (before) url += `&startTime=${before}`;
...
export const getOrderBook = async (instId = 'BTC-USDT', sz = 20) => {
  const symbol = formatSymbol(instId);
  const response = await client.get(`/api/v3/depth?symbol=${symbol}&limit=${sz}`);
  return {
    code: '0',
    data: [{
...
export const getAccountBalance = async () => {
  ...
  return {
    code: '0',
    data: [{
      details: response.data.balances.filter((b) => parseFloat(b.free) > 0 || parseFloat(b.locked) > 0),
    }],
  };
};
...
export const placeOrder = async (instId, side, ordType, sz, px = null) => {
  ...
  let queryString = `symbol=${symbol}&side=${side.toUpperCase()}&type=${ordType.toUpperCase()}&quantity=${sz}&timestamp=${timestamp}`;
  if (px && ordType.toUpperCase() === 'LIMIT') {
    queryString += `&price=${px}&timeInForce=GTC`;
  }
  ...
  return {
    ...
    data: [{
      ...
      clOrdId: response.data.clientOrderId,
    }],
  };
};
...
export const cancelOrder = async (instId, ordId) => {
  const symbol = formatSymbol(instId);
  const timestamp = Date.now();
  const queryString = `symbol=${symbol}&orderId=${ordId}&timestamp=${timestamp}`;
  const signature = generateSignature(queryString);
  ...
  return {
    code: '0',
    data: [{ ordId: response.data.orderId }],
  };
};
